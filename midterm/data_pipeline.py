...
from data_utils.processing import validate_table
...
with DAG(
    ...
    def task_ingest_data(**context):
        ...
        golden_db_uri = conf.get("golden_database_uri") 
        golden_table_name = conf.get("golden_table_name")

        # --- Get unique key definition ---
        # unique_key_input = conf.get("unique_key_column")
        # if not unique_key_input:
        #      raise ValueError("Missing 'unique_key_column' in DAG parameters.")
        # unique_key_columns = [key.strip() for key in unique_key_input.split(',') if key.strip()]
        # if not unique_key_columns:
        #     raise ValueError("Invalid 'unique_key_column' format in DAG parameters.")
    ...
    def task_engineer_features(**context):
        ...
        conf = context["dag_run"].conf
        golden_db_uri = conf.get("golden_database_uri") # Use URI param
        golden_table_name = conf.get("golden_table_name")
        feature_store_uri = conf.get("feature_store_uri") # Use URI param
    ...
    # ingest_task >> validate_task >> features_task
